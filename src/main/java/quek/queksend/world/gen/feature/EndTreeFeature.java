...
public class EndTreeFeature extends Feature<TreeFeatureConfig> {

    public EndTreeFeature(Codec<TreeFeatureConfig> codec) {
        super(codec);
    }
    ...
    private static boolean isPlantableBlock(TestableWorld world, BlockPos pos) {
        return world.testBlockState(pos, (state) -> {
            Block block = state.getBlock();
            return block.isIn(QEBlockTags.generic_end_plantable);
        });
    }
    ...
    private boolean generate(ModifiableTestableWorld world, Random random, BlockPos pos, Set<BlockPos> logPositions, Set<BlockPos> leavesPositions, BlockBox box, TreeFeatureConfig config) {
        ...
        if (blockPos2.getY() >= 1 && blockPos2.getY() + i + 1 <= 256) {
            if (!isPlantableBlock(world, blockPos2.down())) {
                return false;
            } else {
        ...
        while(var9.hasNext()) {
            ...
            for(int var13 = 0; var13 < var12; ++var13) {
                ...
                if (!logs.contains(mutable)) {
                    ...
                    if (blockState.contains(net.minecraft.state.property.Properties.DISTANCE_1_7)) {
                        ((Set)list.get(0)).add(mutable.toImmutable());
                        setBlockStateWithoutUpdatingNeighbors(world, mutable, (BlockState)blockState.with(net.minecraft.state.property.Properties.DISTANCE_1_7, 1));
            ...
            while(var25.hasNext()) {
                ...
                for(int var16 = 0; var16 < var28; ++var16) {
                    ...
                    if (!set.contains(mutable) && !set2.contains(mutable)) {
                        ...
                        if (blockState2.contains(net.minecraft.state.property.Properties.DISTANCE_1_7)) {
                            int l = (Integer)blockState2.get(net.minecraft.state.property.Properties.DISTANCE_1_7);
                            if (l > k + 1) {
